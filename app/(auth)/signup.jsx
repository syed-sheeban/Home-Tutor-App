...
export default function SignupScreen() {
  ...
  const handleMessage = async (event) => {
    try {
      ...
      if (data && data.type === 'AUTH_SUCCESS' && !handledAuthRef.current) {
        ...
        const { user, token, isVerified, tutorApplicationRequired, role } = data;
        const userWithRole = {
          ...user,
          role: user?.role || role || requestedRole,
        };
        const normalizedRole = String(userWithRole.role || '').toUpperCase();
        const requiresTutorApplication =
          normalizedRole === 'TUTOR' ? true : tutorApplicationRequired;
        
        // Save the authenticated user in the store
        await loginStore(
          userWithRole,
          token,
          normalizedRole === 'TUTOR' ? false : isVerified,
          requiresTutorApplication,
        );
        setSuccessUser(userWithRole);
      }
    } catch (err) {
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <AuthSuccessModal
        visible={!!successUser}
        title={isTutorSignup ? "Tutor Registration Successful" : "Sign Up Successful"}
        message={
          isTutorSignup
            ? 'Fill out the tutor application form to become a verified tutor. Your dashboard will unlock after admin approval.'
            : `Welcome, ${successUser?.fullName || successUser?.name || 'there'}`
        }
        buttonLabel={isTutorSignup ? "Fill Tutor Form" : "Continue"}
        onClose={() => {
          setSuccessUser(null);
          router.replace(isTutorSignup ? '/(public)/become-tutor' : '/(tabs)/home');
        }}
      />
    </View>
  );
